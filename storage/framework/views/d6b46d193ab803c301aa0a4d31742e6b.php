<div class="relative z-20">
    ...
    <nav class="fixed top-4 left-1/2 -translate-x-1/2 w-[96%] md:w-[90%] z-50 transition-all duration-300
        ...
        <div class="flex lg:hidden items-center justify-center flex-1 gap-1 sm:gap-4 mx-1">
            ...
            <!-- Torneos -->
            <?php if(!auth()->user()->hasRole('Master Admin')): ?>
                <a href="<?php echo e(route('tournaments.index')); ?>" class="p-2 rounded-full <?php echo e(request()->routeIs('tournaments.*') ? 'text-orange-600 bg-orange-50' : 'text-gray-500 hover:bg-gray-100'); ?> transition-colors" title="Torneos">
            ...
            <?php if(!auth()->user()->hasRole('Master Admin') && !auth()->user()->hasRole('Arbitro')): ?>
                <a href="<?php echo e(route('games.index')); ?>" class="p-2 rounded-full <?php echo e(request()->routeIs('games.index', 'games.create', 'games.*') ? 'text-orange-600 bg-orange-50' : 'text-gray-500 hover:bg-gray-100'); ?> transition-colors" title="Partidos Manuales">
            ...
            <!-- Equipos -->
            <?php if(!auth()->user()->hasRole('Master Admin') && !auth()->user()->hasRole('Arbitro')): ?>
                <a href="<?php echo e(route('teams.index')); ?>" class="p-2 rounded-full <?php echo e(request()->routeIs('teams.*') ? 'text-orange-600 bg-orange-50' : 'text-gray-500 hover:bg-gray-100'); ?> transition-colors" title="Equipos">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
            ...
            <?php if(!auth()->user()->hasRole('Master Admin') && !auth()->user()->hasRole('Arbitro') && !auth()->user()->hasRole('Coach')): ?>
                <a href="<?php echo e(route('players.index')); ?>" class="p-2 rounded-full <?php echo e(request()->routeIs('players.*') ? 'text-orange-600 bg-orange-50' : 'text-gray-500 hover:bg-gray-100'); ?> transition-colors" title="Jugadores">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
            ...
            <?php if(auth()->user()->hasRole('Master Admin')): ?>
                <a href="<?php echo e(route('clients.index')); ?>" class="p-2 rounded-full <?php echo e(request()->routeIs('clients.*') ? 'text-orange-600 bg-orange-50' : 'text-gray-500 hover:bg-gray-100'); ?> transition-colors" title="Panel Admin">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
        ...
        <div class="hidden lg:flex items-center space-x-6">
            ...
            <?php if(!auth()->user()->hasRole('Master Admin')): ?>
                <a href="<?php echo e(route('tournaments.index')); ?>" class="text-sm font-semibold text-gray-600 hover:text-orange-500 transition-colors <?php echo e(request()->routeIs('tournaments.*') ? 'text-orange-600' : ''); ?>">
                    Torneos
                </a>
            <?php endif; ?>

            <?php if(!auth()->user()->hasRole('Master Admin')): ?>
                <a href="<?php echo e(route('games.index')); ?>" class="text-sm font-semibold text-gray-600 hover:text-orange-500 transition-colors <?php echo e(request()->routeIs('games.*') ? 'text-orange-600' : ''); ?>">
                    Partidos
                </a>
            <?php endif; ?>

            <?php if(!auth()->user()->hasRole('Master Admin') && !auth()->user()->hasRole('Arbitro')): ?>
                <a href="<?php echo e(route('teams.index')); ?>" class="text-sm font-semibold text-gray-600 hover:text-orange-500 transition-colors <?php echo e(request()->routeIs('teams.*') ? 'text-orange-600' : ''); ?>">
                    Equipos
                </a>
            <?php endif; ?>

            <?php if(!auth()->user()->hasRole('Master Admin') && !auth()->user()->hasRole('Arbitro') && !auth()->user()->hasRole('Coach')): ?>
                <a href="<?php echo e(route('players.index')); ?>" class="text-sm font-semibold text-gray-600 hover:text-orange-500 transition-colors <?php echo e(request()->routeIs('players.*') ? 'text-orange-600' : ''); ?>">
                    Jugadores
                </a>
            <?php endif; ?>

            <?php if(auth()->user()->hasRole('Master Admin')): ?>
                <a href="<?php echo e(route('clients.index')); ?>" class="text-sm font-semibold text-gray-600 hover:text-orange-500 transition-colors <?php echo e(request()->routeIs('clients.*') ? 'text-orange-600' : ''); ?>">
                    Panel
                </a>
            <?php endif; ?>
        </div>
            ...
            <?php if(!auth()->user()->hasRole('Master Admin') && !auth()->user()->hasRole('Arbitro') && !auth()->user()->hasRole('Coach')): ?>
                <div class="relative config-dropdown-container">
                    ...
                    <div id="config-desktop-menu" class="hidden absolute right-0 mt-2 w-48 bg-white rounded-xl shadow-lg py-1 ring-1 ring-black ring-opacity-5 z-50 transition-all duration-200 ease-out origin-top-right transform scale-95 opacity-0 border border-gray-100">
                        <a href="<?php echo e(route('users.index')); ?>" class="block px-4 py-2 text-sm text-gray-700 hover:bg-orange-50 <?php echo e(request()->routeIs('users.*') ? 'bg-orange-50 text-orange-600 font-bold' : ''); ?>">Usuarios</a>
                        <a href="<?php echo e(route('courts.index')); ?>" class="block px-4 py-2 text-sm text-gray-700 hover:bg-orange-50 <?php echo e(request()->routeIs('courts.*') ? 'bg-orange-50 text-orange-600 font-bold' : ''); ?>">Canchas</a>
                        <a href="<?php echo e(route('strengths.index')); ?>" class="block px-4 py-2 text-sm text-gray-700 hover:bg-orange-50 <?php echo e(request()->routeIs('strengths.*') ? 'bg-orange-50 text-orange-600 font-bold' : ''); ?>">Fuerzas</a>
                        <div class="h-px bg-gray-100 my-1 mx-2"></div>
                        <a href="<?php echo e(route('profile.edit')); ?>" class="block px-4 py-2 text-sm text-gray-700 hover:bg-orange-50 <?php echo e(request()->routeIs('profile.edit') ? 'bg-orange-50 text-orange-600 font-bold' : ''); ?>">Mi Perfil</a>
                    </div>
                </div>
            <?php endif; ?>

            
            <form action="<?php echo e(route('logout')); ?>" method="POST" onsubmit="return confirm('¿Estás seguro de que quieres salir del sistema?');">
                <?php echo csrf_field(); ?>
...
</script><?php /**PATH C:\Users\luism\gemini-work\sistemaTorneos\resources\views/layouts/navigation.blade.php ENDPATH**/ ?>