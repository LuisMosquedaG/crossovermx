    ...
    <div class="mb-6 flex flex-col md:flex-row items-center gap-4">
        ...
        @if(auth()->user()->hasRole('Super Admin'))
            ...
            <div class="relative w-full md:flex-1 flex items-stretch h-10">
                ...
                <form action="{{ route('users.index') }}" method="GET" class="relative flex-1 h-full">
                ...
                @if ($viewTrash)
                    <a href="{{ route('users.index') }}" class="inline-flex items-center justify-center h-full px-4 border border-l-0 border-gray-300 rounded-r-md bg-orange-100 text-orange-700 hover:bg-orange-200 focus:outline-none focus:ring-2 focus:ring-orange-500 transition duration-150 ease-in-out" title="Ver Usuarios Activos">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M19.5 14.25v-2.625a3.375 3.375 0 0 0-3.375-3.375h-1.5A1.125 1.125 0 0 1 13.5 7.125v-1.5a3.375 3.375 0 0 0-3.375-3.375H8.25m0 12.75h7.5m-7.5 3H12M10.5 2.25H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 0 0-9-9Z" />
                        </svg>
                    </a>
                @else
                    <a href="{{ route('users.index', ['trashed' => 'yes']) }}" class="inline-flex items-center justify-center h-full px-4 border border-l-0 border-gray-300 rounded-r-md bg-white text-gray-400 hover:text-red-600 hover:bg-red-50 focus:outline-none focus:ring-2 focus:ring-gray-300 transition duration-150 ease-in-out" title="Ver Papelera">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
        ...
        @else
            ...
            <form action="{{ route('users.index') }}" method="GET" class="relative w-full md:flex-1 h-10">
                <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                    <svg class="h-5 w-5 text-gray-400" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
                        <path fill-rule="evenodd" d="M9 3.5a5.5 5.5 0 100 11 5.5 5.5 0 000-11zM2 9a7 7 0 1112.452 4.391l3.328 3.329a.75.75 0 11-1.06 1.06l-3.329-3.328A7 7 0 012 9z" clip-rule="evenodd" />
                    </svg>
        ...
        @endif
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-center">
                                        <div class="flex items-center justify-center space-x-3">
                                            ...
                                            @if($user->trashed())
                                                ...
                                                <!-- Restaurar -->
                                                <form action="{{ route('users.restore', $user) }}" method="POST" class="inline" onsubmit="return confirm('¿Restaurar este usuario?');">
                                                    @csrf
                                                ...
                                                <form action="{{ route('users.destroy', $user) }}" method="POST" onsubmit="return confirm('ADVERTENCIA: Esto eliminará el usuario PERMANENTEMENTE de la base de datos. ¿Continuar?');" class="inline">
                                                    @csrf @method('DELETE')
                                            ...
                                            @else
                                                ...
                                                <!-- Editar -->
                                                <button onclick="openEditModal({{ $user->toJson() }})" class="text-indigo-600 hover:text-indigo-900" title="Editar">
                                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
                                                ...
                                                <!-- Enviar a Papelera -->
                                                @if($user->id !== auth()->id())
                                                    <form action="{{ route('users.destroy', $user) }}" method="POST" onsubmit="return confirm('¿Enviar a la papelera?');" class="inline">
                                                        @csrf @method('DELETE')
                            ...
                            @empty
                                <tr>
                                    ...
                                    <td colspan="4" class="px-6 py-4 text-center text-gray-500">
                                        {{ $viewTrash ? 'No hay usuarios en la papelera.' : 'No hay usuarios registrados.' }}
                                    </td>
                                </tr>
                            @endforelse
                        </tbody>
                ...
                <div class="mt-4">
                    {{ $users->appends(['search' => $search, 'trashed' => $viewTrash ? 'yes' : null])->links() }}
                </div>
            </div>
        </div>
    </div>
                    ...
                    <div class="bg-white px-4 pb-4 pt-5 sm:p-6 sm:pb-4">
                        <div class="sm:flex sm:items-start">
                            <div class="w-full">
                                ...
                                <div class="mb-4">
                                    <x-input-label for="modal_email" :value="__('Usuario (ej. juan.perez)')" />
                                    
                                    <div class="relative mt-1">
                                        <!-- INPUT NORMAL -->
                                        <x-text-input id="modal_email" class="block w-full focus:border-orange-500 focus:ring-orange-500" type="text" name="email" required placeholder="Escribe solo tu usuario" />
                                        
                                        <!-- EL TEXSTO DEL DOMINIO VA EN UN SPAN FUERA DEL INPUT -->
                                        <div class="absolute inset-y-0 right-0 flex items-center pr-3 pointer-events-none">
                                            <span class="text-gray-500 text-sm">
                                                {{ optional(auth()->user()->client)->prefix ?? 'dominio.com' }}
                                            </span>
                                        </div>
                                    </div>
                                ...
                                <div class="mb-4">
                                    <x-input-label for="modal_password" :value="__('Contraseña')" />
                                    <!-- CAMBIO: Agregado focus naranja -->
                                    <x-text-input id="modal_password" class="block mt-1 w-full focus:border-orange-500 focus:ring-orange-500" type="password" name="password" required />
                                    <p class="text-xs text-gray-500 mt-1">Mínimo 8 caracteres.</p>
                                </div>

                                <div class="mb-4">
                                    <x-input-label for="modal_password_confirmation" :value="__('Confirmar Contraseña')" />
                                    <!-- CAMBIO: Agregado focus naranja -->
                                    <x-text-input id="modal_password_confirmation" class="block mt-1 w-full focus:border-orange-500 focus:ring-orange-500" type="password" name="password_confirmation" required />
                                </div>
                                
                                <!-- Campo de Rol -->
                                <div class="mb-4">
                                    <x-input-label for="modal_role" :value="__('Rol')" />
...
<script>
    ...
    function openEditModal(user) {
        ...
        document.getElementById('userForm').action = '{{ route("users.update", ":id") }}'.replace(':id', user.id);
